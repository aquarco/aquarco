...
- New test in `supervisor/python/tests/test_docker_version_consistency.py` asserting that `docker/compose.yml` and `docker/compose.prod.yml` stay in sync on the `pgdata` mount path, and that the path is `/var/lib/postgresql` (pg18 layout) rather than the legacy `/var/lib/postgresql/data`.
