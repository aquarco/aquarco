...
class TestExactBugScenarioIssue111:
    ...
    async def test_implement_review_implement_revisit(self) -> None:
        ...
        assert executor._tq.record_stage_executing.call_count == 1
        ...
        executor._tq.get_stage_structured_output = AsyncMock(return_value=impl_output)
        executor._tq.record_stage_executing = AsyncMock()
        executor._tq.create_rerun_stage = AsyncMock()
        executor._registry.increment_agent_instances = AsyncMock()
...
class TestGuardPreventsDownstreamCalls:
    ...
    async def test_no_downstream_calls_on_completed_guard(self) -> None:
        ...
        executor._tq.record_stage_executing = AsyncMock()
        executor._tq.store_stage_output = AsyncMock()
        executor._tq.record_stage_failed = AsyncMock()
        executor._tq.create_rerun_stage = AsyncMock()
        executor._registry.increment_agent_instances = AsyncMock()
        executor._registry.decrement_agent_instances = AsyncMock()
        executor._execute_agent = AsyncMock()
        ...
        executor._tq.record_stage_failed.assert_not_called()
        executor._tq.create_rerun_stage.assert_not_called()
        executor._registry.increment_agent_instances.assert_not_called()
...
class TestGuardIdempotency:
    ...
    async def test_three_consecutive_calls_return_same_output(self) -> None:
        ...
        executor._tq.get_stage_structured_output = AsyncMock(return_value=cached)
        ...
        assert executor._tq.get_latest_stage_run.call_count == 3
        assert executor._tq.get_stage_structured_output.call_count == 3
        # Agent NEVER called
        executor._tq.record_stage_executing.assert_not_called()
...
class TestGuardDoesNotBlockFirstRun:
    ...
    async def test_non_completed_statuses_still_execute(self, status: str) -> None:
        ...
        executor._tq.get_latest_stage_run = AsyncMock(return_value=latest)
        executor._tq.create_rerun_stage = AsyncMock(return_value=701)
        executor._tq.record_stage_executing = AsyncMock()
        executor._registry.increment_agent_instances = AsyncMock()
        executor._registry.decrement_agent_instances = AsyncMock()
        executor._execute_agent = AsyncMock(return_value={
...
class TestStageKeyInGuardPath:
    ...
    async def test_stage_key_format(self) -> None:
        ...
        call_args = executor._tq.get_latest_stage_run.call_args
    ...
    async def test_stage_key_with_hyphenated_names(self) -> None:
        ...
        stage_key = executor._tq.get_latest_stage_run.call_args[0][1]
